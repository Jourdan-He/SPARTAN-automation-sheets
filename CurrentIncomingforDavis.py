...
def getpartMTL():
    ...
    while(i<len(SitesCode)):
        if 'Lab' in SitesCode[i]:
            ...
            partMTL=pd.concat([partMTL,MTL_masses[MTL_masses['CartridgeID']==UCDavis[i]]]) #all columns corresponding to Cartridge number given, dataframe
...
if __name__ == "__main__":
    ...
    massCollect=partMTL['Net_Weight_ug'].rename("Mass collected on filter (ug)") # a series to record mass collected on filter
    ...
    startDate=part_dates_flows['start_month']+"/"+part_dates_flows['start_day']+"/"+part_dates_flows['start_year']
    ...
    UCDavis=pd.concat([UCDavis,Barcode,FilterID,AnalysisID,FilterType,ProjectID,startDate,endDate,massCollect,sampleVolume],axis=1)
    UCDavis["Shipment ID (Date)"]=""
    UCDavis["Lot ID"]=""
    UCDavis["Comments"]=""
    UCDavis=UCDavis[['Shipment ID (Date)','Cartridge Number','Barcode','Filter ID','Analysis ID','Filter Type','Project ID','Lot ID','Sampling Start Date','Sampling End Date','Mass collected on filter (ug)','Sampled volume (m3)','Comments']]
    UCDavis.to_excel(f'/storage1/fs1/rvmartin/Active/SPARTAN-shared/Analysis_Data/FTIR/UCDavis_samples_log/{year}/next_UCDavis_shipment_{d}.xlsx',index=False,na_rep='NaN')
